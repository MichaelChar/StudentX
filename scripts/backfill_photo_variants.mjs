  ...
  if (rollbackLines.length > 0) {
    ...
    writeFileSync(
      ...
      `-- Generated by backfill_photo_variants.mjs at ${new Date().toISOString()}\n` +
        ...
        `--\n` +
        `-- DOES NOT clean up Storage. The variant files written by the\n` +
        `-- backfill (__thumb / __card / __full) remain in the listing-photos\n` +
        `-- bucket as orphans after this rollback runs. The eventual\n` +
        `-- originals-reaping follow-up is the right place to sweep those\n` +
        `-- up; do not assume Storage is in pre-backfill state just because\n` +
        `-- this SQL ran successfully.\n\n` +
        rollbackLines.join('\n') +
        '\n'
    );
