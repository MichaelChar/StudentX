...
**University distance coverage (`university-distance-coverage`, issue #549):**
- Asserts that **every university in `DEFAULT_CITY` can be located from a map pin**. Reads the expected set from the `universities` table (never a hardcoded list, so a university added later without coordinates fails rather than quietly measuring as a shorter list), then runs the production `computeUniversityDistances()` over the same two sources the landlord route reads: `faculties` rows first, `universities.lat/lng` as the fallback for universities that have none.
- Fails when any university is unmeasurable, or a distance is `<= 0` (the Null Island signature of a row with no coordinates) or above `MAX_DISTANCE_METERS`.
- **Runs with `useOsrm: false`.** The question is whether every university has a *position* — a data question. Routing them through the public OSRM demo server every 15 minutes would add an external dependency whose outage is not the failure being watched for, and `computeUniversityDistances` falls back to haversine on an OSRM failure anyway, so the verdict would not change. Straight-line metres are enough to prove coverage.
- **Why this check exists.** The landlord wizard's universities step is read-only (PR #542): it lists one row per city university, each measured from the pin, with nothing for the landlord to type. Prod's `faculties` table held 13 rows, *all AUTH*, and university positions came from that table alone — so prod could measure exactly one university, the step showed two "Not measured" rows, and its two-distance minimum then refused to let any new listing past step 4. Nothing caught it: `e2e/specs/06-landlord-wizard.spec.js` **stubs** the distance API for determinism, and the unit tests pass their own fixtures in. Only a call against production data could see it, and nothing made one. Fixed in PR #545 (migration 119); this check is the part that makes the next occurrence visible.
