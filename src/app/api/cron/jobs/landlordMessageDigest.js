...
// (migration 033). The CRON_SECRET check gates the route; this client
// gates the database. Mirrors src/lib/metrics/supabase.js.
...
/**
 * Per-message landlord digest. Returns a result object (never throws for
 * per-row failures — those are counted in `failures`).
 */
export async function runLandlordMessageDigest() {
  // Kill switch: deploy-time disable without removing the job registry entry.
  ...
  if (fetchError) {
    ...
    return { error: 'Failed to fetch pending notifications', ok: false };
  }
  ...
  for (const row of pending) {
    try {
      ...
      await resend.emails.send({
        from: fromAddressFor(row.landlord_name),
