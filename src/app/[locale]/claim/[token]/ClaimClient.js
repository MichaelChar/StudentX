  ...
  return (
    <div className="flex gap-2 overflow-x-auto pb-1">
      {urls.map((u, i) => (
        ...
        <img key={i} src={u} alt="" className="h-40 w-56 object-cover rounded-photo bg-parchment flex-shrink-0" />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <section className="border border-night/10 rounded-card p-4 mb-6">
        ...
        <div className="grid sm:grid-cols-3 gap-2">
          {['display_name', 'phone', 'email'].map((f) => (
            <input
              key={f}
              className="text-sm border border-night/10 rounded-control px-2 py-1.5"
      ...
      {claimable.length === 0 ? (
        ...
        claimable.map((l) => (
          <div key={l.id} className="border border-night/10 rounded-card p-4 mb-4">
            ...
            <textarea
              className="w-full text-sm border border-night/10 rounded-control px-2 py-1.5 mt-2"
              rows={3}
      ...
      <button
        ...
        className="w-full text-base bg-blue text-white rounded-control px-5 py-3 hover:bg-blue/90 active:bg-blue/80 transition-colors disabled:opacity-40"
      >
